...
class EmployAuthenctication(ModelBackend):  # requires to define two functions authenticate and get_user

    def authenticate(self, request, employID=None, password=None, **kwargs):
        UserModel = get_user_model()
        try:
            # below line gives query set,you can change the queryset as per your requirement
            user = UserModel.objects.filter(
                Q(pf_number__iexact=employID) |
                Q(pf_number__iexact=employID)
            ).distinct()
